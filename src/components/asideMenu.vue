      ...
      <div class="wraps">
        <label>
          縣市：<select v-model="currCity">
              <option v-for="c in cityList" :key="c">{{ c }}</option></select>
        </label>
        <label>
          行政區：<select v-model="currDistrict">
              <option v-for="d in districtList" :key="d.id">{{ d.name }}</option></select>
        </label>
      </div>
...
export default {
    ...
    methods: {
      keywordHighlight(val){
          return val.replace(new RegExp(this.keywords), `<span class="highlight">${this.keywords}</span>`)
      },
      openInfoBox(sid){
          this.showModal = true;
          this.infoBoxSid = sid;
